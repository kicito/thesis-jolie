...
Running a Jolie interpreter instance always involving a translation of a service abstraction into an OOIT, before it can begin the execution.
For this reason, along with the introduction of module system where every service is encapsulate in a service node, which make it is ambiguous for an interpreter to decided which service node to be an execution target, there is a requirement of the target service node  when a module is called as a execution target. Hereby, we introduce the definition of an executable Jolie module so called \texttt{Jolie script}.

A Jolie script is a Jolie module that included a service definition named \texttt{main}, which can be executed from the command
